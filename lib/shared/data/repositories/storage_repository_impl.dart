...
import "../../domain/entities/storage_key.dart";
import "../../domain/repositories/storage_repository.dart";

class StorageRepositoryImpl {
  const StorageRepositoryImpl({required this._secure, required this._regular});

  final StorageRepository _secure;
  final StorageRepository _regular;

  // Routing automatique selon la clé
  StorageRepository _pick(StorageKey key) => key.secure ? _secure : _regular;
  ...
  Future<String?> read(StorageKey key) => _pick(key).read(key.name);

  Future<void> delete(StorageKey key) => _pick(key).delete(key.name);
